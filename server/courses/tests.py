...
class CourseMaterialTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.detail_url = reverse(
            "course-materials-detail", args=[self.course.id, self.material.id]
        )
    ...
    def test_upload_material(self):
        ...
        data = {"title": "New Material", "file": self.generate_test_file()}
    ...
    def test_material_visibility(self):
        """Test material visibility based on course activation"""
...
class EnrollmentTests(APITestCase):
    ...
    def test_bulk_unenrollment(self):
        ...
        response = self.client.delete(
            self.list_url, {"student_ids": [self.student.id]}, format="json"
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(Enrollment.objects.filter(id=enrollment.id).exists())

    def test_enrollment_in_inactive_course(self):
        """Test student cannot enroll in an inactive course"""
        self.course.is_active = False
        self.course.save()
        ...
        self.assertEqual(response.data["error"], "Cannot enroll in an inactive course")

    def test_duplicate_enrollment(self):
        """Test student cannot enroll in same course multiple times"""
        EnrollmentFactory(student=self.student, course=self.course)
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error"], "Already enrolled in this course")

    def test_unenrollment_not_enrolled(self):
        """Test student cannot unenroll from course they're not enrolled in"""
        self.client.force_authenticate(user=self.student)
        response = self.client.delete(self.enroll_url, format="json")

        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data["error"], "Not enrolled in this course")
...
class FeedbackTests(APITestCase):
    ...
    def test_delete_feedback(self):
        ...
        url = reverse("course-feedback-detail", args=[self.course.id, feedback.id])
        response = self.client.delete(url)

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Feedback.objects.count(), 0)

    def test_feedback_character_limit(self):
        """Test feedback character limit validation"""
        self.client.force_authenticate(user=self.student)

        # Create feedback with more than 1000 characters
        long_comment = "a" * 1001
        data = {"comment": long_comment}

        response = self.client.post(self.list_url, data, format="json")

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            "Ensure this field has no more than 1000 characters", str(response.data)
        )
